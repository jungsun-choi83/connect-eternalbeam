...
import { bindAnonymousToAccount, exchangeKakaoOAuthCode } from '../lib/authApi'
...
export function OAuthKakaoCallbackPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        await exchangeKakaoOAuthCode(code)
  ...
  return (
    <div className="flex min-h-dvh flex-col items-center justify-center bg-[#191919] text-[#FEE500]">
      <p className="font-sans text-sm tracking-wide">카카오 계정으로 연결하는 중…</p>
    </div>
  )
}
